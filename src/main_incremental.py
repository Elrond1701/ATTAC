...
def main(argv=None):
    ...
    parser.add_argument('--results-path', type=str, default='../results_ViT_ATT_10splits_cifar100_224',
                        help='Results path (defaultSs=%(default)s)')
    ...
    parser.add_argument('--num-workers', default=24, type=int, required=False,
    ...
    # model args
    parser.add_argument('--network', default='OVit_tiny_16_augreg_224', type=str, choices=allmodels,
    ...
    # training args
    parser.add_argument('--approach', default='olwf_asym_original', type=str, choices=approach.__all__,
    ...
    parser.add_argument('--lr-min', default=1e-8, type=float, required=False,
    ...
    bwt_taw = np.zeros((max_task))
    bwt_tag = np.zeros((max_task))
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        for u in range(t + 1):
            ...
            print('>>> Test on task {:2d} : loss={:.3f} | TAw acc={:5.1f}%, forgtaw={:5.1f}% '\
                  '| TAg acc={:5.1f}%, forgtag={:5.1f}% <<<'.format(u, test_loss,100 * acc_taw[t, u], 100 * forg_taw[t, u], 100 * acc_tag[t, u], 100 * forg_tag[t, u]))
        ...
        if t > 0:
           bwt_taw[t] = (acc_taw[t, :t] - acc_taw[t-1, :t]).mean()
           print("bwt_taw[t]", bwt_taw[t])
           bwt_tag[t] = (acc_tag[t, :t] - acc_tag[t-1, :t]).mean()      
           print("bwt_tag[t]", bwt_tag[t])
        # Save
        ...
        arrtag = np.tril(np.ones(forg_tag.shape[0])).sum(1)
        arrtag[1:] = arrtag[1:] - 1

        arrtaw = np.tril(np.ones(forg_taw.shape[0])).sum(1)
        arrtaw[1:] = arrtaw[1:] - 1


        avg_forg_taw = np.sum(np.tril(forg_taw, k=-1), axis=1) / arrtaw
        avg_forg_tag = np.sum(np.tril(forg_tag, k=-1), axis=1) / arrtag


        
        # Log average forgetting
        logger.log_result(avg_forg_taw, name="avg_forg_taw", step=t)
        logger.log_result(avg_forg_tag, name="avg_forg_tag", step=t)

        print("avg_forg_taw:",avg_forg_taw)
        print("arrtaw",  arrtaw) 
        print("np.sum(np.tril(forg_taw, k=-1), axis=1):",  np.sum(np.tril(forg_taw, k=-1), axis=1))
