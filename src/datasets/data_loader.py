...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    elif 'PathMNIST' == dataset:
        tvcifar_trn = PathMNIST(split="train", download=True)
        tvcifar_tst = PathMNIST(split="test", download=True)
        trn_data = {'x': tvcifar_trn.imgs, 'y': tvcifar_trn.labels}
        tst_data = {'x': tvcifar_tst.imgs, 'y': tvcifar_tst.labels}
    ...
    elif 'ChestMNIST' == dataset:
        tvcifar_trn = ChestMNIST(split="train", download=True)
        tvcifar_tst = ChestMNIST(split="test", download=True)
        trn_data = {'x': tvcifar_trn.imgs, 'y': tvcifar_trn.labels}
        tst_data = {'x': tvcifar_tst.imgs, 'y': tvcifar_tst.labels}
