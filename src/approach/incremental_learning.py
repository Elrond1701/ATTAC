...
class Inc_Learning_Appr:
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
        ...
        self.optimizer = torch.optim.Adamax(self.model.parameters(), lr=self.lr, weight_decay=self.wd)
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=30, eta_min=0, last_epoch=-1, verbose=True)
    ...
    def _get_optimizer(self):
        ...
        # return torch.optim.Adamax(self.model.parameters(), lr=self.lr, weight_decay=self.wd)
    ...
    #     scaler = GradScaler()  # 创建梯度缩放器对象
    
    #     for e in range(self.nepochs):
    #         clock0 = time.time()
            
    #         self.train_epoch(t, trn_loader, scaler)  # 将梯度缩放器作为参数传递给train_epoch函数
    ...
    # def train_epoch(self, t, trn_loader, scaler):
    #     """Runs a single epoch"""
    #     self.model.train()
